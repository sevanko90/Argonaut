...
public sealed class JsonToolbarViewModelTests : IDisposable
{
    ...
    /// so tests have to wait for the parse to land. Its continuation comes back through the
    /// stand-in dispatcher, so the whole update - the document and everything that reacts to
    /// it - has been applied on this thread by the time this returns.</summary>
    private Task WaitForDocumentAsync(JsonSchemaSettings settings)
