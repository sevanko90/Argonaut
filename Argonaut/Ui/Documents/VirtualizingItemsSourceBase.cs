...
/// (<c>CsvRowCollection</c>), raw rows (<c>RawRowCollection</c>), array-table rows
/// (<c>JsonArrayRowCollection</c>) and diff rows (<c>JsonDiffRowCollection</c>). It supplies the read-only, fixed-size
...
/// safety every such collection must get right. Once disposed, <see cref="Count"/> reports 0
...
/// materialises every row of a multi-GB document (a multi-second stall on file close/switch) and
/// dereferences a just-released mapping - a native use-after-free that crashed with an access
...
/// </summary>
public abstract class VirtualizingItemsSourceBase : IList, INotifyCollectionChanged, IDisposable
{
    ...
    /// <summary>
    /// Materialises the item at <paramref name="index"/>, reading the source as needed. Only
    /// ever invoked while live: the base short-circuits to null once disposed, so this never
    /// runs against a released source.
    /// </summary>
    ...
    /// already been flipped to its empty, source-free state (see <see cref="Dispose"/>) before
    /// this runs.
    /// </summary>
