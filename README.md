...
- Any individual JSON document is limited to about 4 GB, due to the internal structural index using 32-bit file offsets. In practice this shouldn't be a real issue - a single 4GB json file is impractical to consume with most standard DOM parsers anyway but it can be revisited if it becomes a real issue.
